...
def getlpGBTHists2D(data, module_hists2D):

    lpgbt_hists2D = []
    
    n_binPhi = module_hists2D[1][0,1,1,1].GetNbinsY() #get bins phi from one of the hists. 
    phi_120deg = module_hists2D[1][0,1,1,1].ProjectionY().GetBinLowEdge(13) #should be 2.0943951023931953 radian, i.e. 120deg

    for p,phiselection in enumerate(module_hists2D):#inclusive and phi < 60
        ...
        lpgbt_hists2D.append(temp)
    
    return lpgbt_hists2D
